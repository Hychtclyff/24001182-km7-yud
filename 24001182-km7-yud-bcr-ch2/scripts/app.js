...
const resultContent = document.getElementById("result-card");
const formDriver = document.getElementById("driver");
const formDate = document.getElementById("date");

const formCount = document.getElementById("count");

function checkEnableSubmit() {
  if (formDriver.value && formDate.value) {
    btnSubmit.disabled = false;
  } else {
    btnSubmit.disabled = true;
  }
}
formDriver.addEventListener("change", checkEnableSubmit);
formDate.addEventListener("change", checkEnableSubmit);

btnSubmit.addEventListener("click", () => {
  const driver = formDriver.value;
  const date = formDate.value;
  const count = formCount.value.toLowerCase();
  const filter = {
    driver,
    count,
    date,
  ...
  searchCarsContent(filter);
});

async function searchCarsContent(filter) {
  ...
  data.map((car) => {
    const card = `
 <div class="card p-4 rounded-2" style="width: 20rem">
            <img src="/assets/img/cars/${car.image.substring(
              car.image.lastIndexOf("/") + 1
            ...
            <ul class="list-group list-group-flush border-0">
              <li
                class="list-group-item d-flex flex-row gap-3 justify-content-start align-items-center border-0"
              >
                <svg
                  width="20"
                  height="20"
            ...
            <div class="card-body">
              <button class="btn btn-success text-white w-100" id="btn-submit">
                Pilih Mobil
              </button>
            </div>
          </div>`;
    resultContentHTML += card;
  });
...
const getCars = async (filter) => {
  ...
  const newData = data.map((e) => {
    ...
    const randomDate = getRandomDate(today, futureDate);

    const formattedDate = randomDate.toISOString();

    e.availableAt = formattedDate;
  ...
  const filteredData = newData.filter((car) => {
    return (
      filter.driver.includes(car.available) &&
      filter.count <= car.capacity &&
      new Date(filter.date) >= new Date(car.availableAt)
    );
  });
